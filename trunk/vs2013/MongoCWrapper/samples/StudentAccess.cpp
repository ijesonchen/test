...
namespace
{
	...
	{
		return MongoClib::EncUtf8(strUnicode.GetString());
	}

	// string (utf-8) -> wstring (unicode)
	CString DecodeUtf8MS(const std::string& strUtf8)
	{
		return MongoClib::DecUtf8(strUtf8).c_str();
	}
}
...
AutoBson StudentAccess::BuildBson(const Student& student)
{
	...
	if (!BsonAddContainer(doc, "vtText", student.vtText, cstring2str))
	{
		return nullptr;
	}
	return std::move(doc);
...
AutoBson StudentAccess::BuildBson(const CString& name)
{
	AutoBson doc;
	if (!doc)
	{
...
void StudentAccess::ParseBson(Student& student, const bson_t* doc)
{
	...
	BsonGetContainer(doc, "vtText", student.vtText, str2cstring);
}
...
bool StudentAccess::CreateIndex(void)
{
	...
	if (!coll.CreateUniqueIndex(string("strName"))) return false;
	if (!coll.CreateIndex(string("fValue"))) return false;

	AutoBson doc1;
	doc1.Add("nValue", 1);
	doc1.Add("fValue", 1);
	if (!coll.CreateIndex(doc1)) return false;

	AutoBson doc2;
	doc2.Add("nValue", 1);
	doc2.Add("fValue", -1);
	if (!coll.CreateIndex(doc2)) return false;
